...
// aligns the query sequence to the reference using the Smith Waterman Gotoh algorithm
void CSmithWatermanGotoh::Align(unsigned int& referenceAl, string& cigarToRealignAl, vector<BamTools::CigarOp>& cigarToRealignbamAl, vector<CigarOp>& cigarbamAl, float& BestScore, int& readlength, int& longestINDELlen, int& longestSlen, float& fractionMismatches, int& NumOfIDS, int& NumOfm, int& longestContiguousMismatch, const char* s1, const unsigned int s1Length, const char* s2, const unsigned int s2Length)
{
	if((s1Length == 0) || (s2Length == 0)) {
		cerr << "ERROR: Found a read with a zero length." << endl;
		exit(1);
	...
	while(keepProcessing) {
		...
		switch(mPointers[ck + cj]) {
			...
			case Directions_DIAGONAL:
				...
				// increment our mismatch counter
				if(mScoringMatrix[c1 - 'A'][c2 - 'A'] == mMismatchScore)
                                {
                                    IsBaseMismatch.push(true);
                                    M_numMismatches++;
                                    numMismatches++;
                                }
                                else
                                    IsBaseMismatch.push(false);
				break;
	...
	*/
	...
	unsigned int alLength = strlen(mReversedAnchor);
	int M = 0, m = 0, d = 0, i = 0, numNonIndelNucleotide = 0;
        int NumofM = 0, Numofm = 0, Numofd = 0, Numofi = 0, Numofs = 0;
	bool dashRegion = false;
	ostringstream oCigar (ostringstream::out);

	if ( cj != 0 )  // Assume mismatches for now
        {
            CigarOp op;
            op.Type = (char)'S';
            op.Length =  cj;
            cigarbamAl.push_back(op);
            cigarToRealignbamAl.push_back(op);
            oCigar << cj << 'S';  // cj = QueryBegin
            Numofs++;
            longestSlen = cj;
            readlength+= cj;
			...
			if ( dashRegion )
				...
				if ( d != 0 )
				...
				else
                                {
                                    ...
                                    if (i > longestINDELlen)
                                        longestINDELlen = i;
                                }
			}
			dashRegion = false;
                        if (IsBaseMismatch.top() == false)  // is Match base
                        {
                            IsBaseMismatch.pop();
                            // Previously is Mismatch Bases, Update Cigars
                            if (m > 0)
                            {
                                CigarOp op;
                                op.Type = (char)'m';
                                ...
                                if ( (cigarbamAl.empty()) || (cigarbamAl.back().Type != 'M'))
                                {
                                    CigarOp cp;
                                    cp.Type = (char)'M';
                                    cp.Length = m;
                                    cigarbamAl.push_back(cp);
                                }
                                else if (cigarbamAl.back().Type == 'M')
                                    cigarbamAl.back().Length += m;

                                numNonIndelNucleotide += m;
                                readlength += m;
                                Numofm++;
                                if (m > longestContiguousMismatch)
                                    longestContiguousMismatch = m;
                                
                                m = 0;
                            }
                            M++;
                        }
                        else    // is Mismatch base
                        {
                            IsBaseMismatch.pop();
                            // Previously is Match Bases, Update Cigars
                            if (M > 0)
                            {
                                CigarOp op;
                                op.Type = (char)'M';
	...
	if ( BestColumn != s2Length ) //Assume mismtaches for now
        {
            //
            CigarOp op;
            op.Type = (char)'S';
            op.Length =  s2Length - BestColumn;
            cigarbamAl.push_back(op);
            cigarToRealignbamAl.push_back(op);
	    oCigar << s2Length - BestColumn << 'S';
            readlength += s2Length - BestColumn;
            Numofs++;

            if (longestSlen < (int)op.Length)
                longestSlen = op.Length;

            //nonindel += s2Length - BestColumn;
            //numMismatches += s2Length - BestColumn;
        }

	cigarToRealignAl = oCigar.str();
        fractionMismatches = (float)M_numMismatches/(float)numNonIndelNucleotide;
        NumOfIDS = Numofi + Numofd + Numofs;
        NumOfm = Numofm;
