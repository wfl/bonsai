...
#ifndef _MODIFIEDSMITHWATERMANGOTOH_H
#define	_MODIFIEDSMITHWATERMANGOTOH_H
...
public:
	...
	// aligns the query sequence to the reference using the Smith Waterman Gotoh algorithm
        // s1 = Reference
        // s2 = Query
	void Align(unsigned int& referenceAl, std::string& cigarAl, const char* s1, const unsigned int s1Length, const char* s2, const unsigned int s2Length);
	void Align(unsigned int& referenceAl, std::string& cigarToRealignAl, std::vector<BamTools::CigarOp>& cigarToRealignbamAl, std::vector<BamTools::CigarOp>& cigarbamAl, float& BestScore, int& readlength, int& longestINDELlen, int& longestSlen, float& fractionMismatches, int& NumOfIDS, int& NumOfm, int& longestContiguousMismatch, const char* s1, const unsigned int s1Length, const char* s2, const unsigned int s2Length);
...
#endif	/* _MODIFIEDSMITHWATERMANGOTOH_H */
